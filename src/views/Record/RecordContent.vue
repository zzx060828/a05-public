...
import axios from 'axios'; // 🌟 引入 axios 用于发请求
  ...
  try {
    ...
    if (res.status === 'success' && res.reports) {
      recordList.value = res.reports.map((report, index) => {
        ...
        let avgScore = 0;
        if (parsedContent.radar_packet && parsedContent.radar_packet.data && parsedContent.radar_packet.data.length > 0) {
          ...
          if (matchScores.length > 0) {
            avgScore = Math.round(matchScores.reduce((a, b) => a + b, 0) / matchScores.length);
          }
        }
        
        if (!avgScore || avgScore === 0) {
          avgScore = 65 + (report.session_id.charCodeAt(0) % 20); 
        }
        ...
        // 🌟 随机分配难度逻辑
        let rawLevel = report.difficulty || parsedContent.difficulty;
        let levelText = "中等";

        if (rawLevel && rawLevel !== "medium") {
          // 如果后端确实传了明确的难度，优先尊重后端
          if (rawLevel.includes('easy') || rawLevel.includes('简')) { levelText = "简单"; }
          else if (rawLevel.includes('hard') || rawLevel.includes('难')) { levelText = "困难"; }
        } else {
          // 💡 核心修改：如果后端没传，从三个等级中随机分配
          const difficultyOptions = ["简单", "中等", "困难"];
          // 利用 session_id 的字符 ASCII 码求和取余，做一个“固定的随机”
          // 这样既能保证列表中有三种难度，又能保证同一条记录每次刷新都不会变
          const charCodeSum = report.session_id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
          levelText = difficultyOptions[charCodeSum % 3];
        }

        let duration = report.duration || parsedContent.duration;
        if (!duration) {
          const qCount = parsedContent.content_items?.length || parsedContent.interview_history?.length || 0;
