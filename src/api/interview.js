...
const apiClient = axios.create({
  ...
  timeout: 60000, // 报告生成可能较慢，建议把超时稍微改长一点（60秒）
});
...
export const submitAnswer = async (formData) => {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/chat_with_audio`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`
        // 🚨 千万别写 Content-Type！fetch 遇到 FormData 会自动加上带 boundary 的 Content-Type
      },
      body: formData
    });
...
// 提交纯文字答案
export const submitTextAnswer = async (data) => {
  try {
...
export const getStreamingReport = async (sessionId, onChunk, onDone) => {
  try {
    ...
    const response = await apiClient.post('/api/report', { session_id: sessionId });
    ...
    if (data.status === 'success') {
        ...
        if (data.radar_packet) onChunk(data.radar_packet);
        if (data.summary_packet) onChunk(data.summary_packet);
        if (data.content_items && data.content_items.length > 0) {
            data.content_items.forEach(item => onChunk(item));
        }
        if (data.voice_data) onChunk({ type: 'voice', data: data.voice_data });
        if (data.video_data) onChunk({ type: 'video', data: data.video_data });
        if (data.tips_data) onChunk({ type: 'encourage', data: data.tips_data });
        if (data.history_trend) onChunk({ type: 'history_trend', data: data.history_trend });
    } else {
