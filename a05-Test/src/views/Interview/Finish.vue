...
import { getStreamingReport } from '@/api/interview'
...
const loadCachedSummary = () => {
  try {
    ...
    if (cachedData) {
      ...
      timeSpentSeconds.value = data.timeSpent || 0
      console.log('已加载缓存数据，等待后端真实评分...')
    }
  } catch (e) {
  ...
  try {
    ...
    if (report.status === 'success') {
      ...
      const realScore = report.average_score
        ...
        || report.summary_packet?.matchScore
        || report.match_score
        || 0
      ...
      // 题量与耗时
      if (report.summary_packet?.totalQuestions) {
        totalCount.value = report.summary_packet.totalQuestions
      }
      if (!answeredCount.value && report.summary_packet?.totalQuestions) {
        answeredCount.value = report.summary_packet.totalQuestions
      }
      if (report.duration) {
        timeSpentSeconds.value = report.duration * 60
      }

      // 分析摘要：从 content_items 提取
      const items = report.content_items || []
      if (items.length > 0) {
        analysisSummaryList.value = items.map((item) => {
          const inner = item.item || item
          if (typeof inner === 'string') return inner
          const q = inner.question || ''
          const f = inner.feedback || inner.analysis || ''
          return `${q} —— ${f}`
        }).filter(s => s.trim().length > 0)
      } else if (report.report_markdown) {
        analysisSummaryList.value = report.report_markdown
          .replace(/\*\*/g, '')
          .split(/[；。\n]/)
          .filter(s => s.trim().length > 0)
      } else {
        analysisSummaryList.value = ['AI 报告已生成，点击下方按钮查看详细分析。']
      }
    } else {
      // 后端返回失败，用缓存兜底
    ...
    console.error('获取真实报告失败，使用缓存兜底:', e)
    loadFromCacheOrEmpty()
  } finally {
    isLoading.value = false
  }
...
const loadFromCacheOrEmpty = () => {
  try {
    ...
    if (cachedData) {
      ...
      // 缓存里的分只作参考，标记为 estimate
      if (data.score && score.value === 0) {
        animateScore(Math.min(data.score, 60))
      }
    } else {
