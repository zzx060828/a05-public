    ...
    res.reports.forEach(report => {
      ...
      if (content.radar_packet && content.radar_packet.data) {
        content.radar_packet.data.forEach(d => {
          const dimName = d.name || d.label
          if (!dimensionSums[dimName]) {
            dimensionSums[dimName] = 0; dimensionCounts[dimName] = 0;
          }
          dimensionSums[dimName] += d.value; dimensionCounts[dimName] += 1;
        })
      }
    })
...
const initRadarChart = () => {
  ...
  const option = {
    ...
    radar: {
      axisName: { color: '#64748b', fontSize: 13, fontWeight: 'normal' },
