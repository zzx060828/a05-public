  ...
  try {
    ...
    if (res.status === 'success' && res.reports) {
      ...
      const allRecords = res.reports.map(report => {
        ...
        else if (textToSearch.includes('前端') || textToSearch.includes('frontend') || textToSearch.includes('web')) jobStr = "Web前端工程师";
        else if (textToSearch.includes('后端')) jobStr = "后端开发工程师";

        let avgScore = parsed.average_score || 0;
        if (!avgScore && parsed.radar_packet?.data?.length > 0) {
          const scores = parsed.radar_packet.data.map(d => d.value).filter(v => v > 0);
          avgScore = scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 0;
        }
        if (!avgScore) avgScore = parsed.match_score || 0;
